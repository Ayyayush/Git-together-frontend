# 🚀 Lecture 2 – Frontend Authentication & State Management (GitTogether)
...
## 🔗 Binding Form Fields to State Variables
...
## 🔁 Fetch vs Axios (Conceptual Difference)
...
- Login will appear successful but user won’t stay logged in
...
## 🤔 Why User Slice is Needed

Without user slice:
- Navbar can’t know if user is logged in
- Profile image can’t be shown
...
## 🖼️ Showing User Photo in Navbar
...
- Login page → Controller trigger
- Axios → API client
- Redux store → In-memory database
- Slice → Schema-like structure
- CORS → Security layer
