...
import { 
  ...
  FaGithub, FaLinkedin, FaGlobe, FaFileAlt, FaLock, FaTimes
} from "react-icons/fa";
...
const LeftSidebar = ({ setShowPremiumModal, onCloseDrawer }) => {
  ...
  // Real dynamic data hook subscriptions
  const connectionsList = useSelector((state) => state.connections);
  const pendingRequestsList = useSelector((state) => state.requests?.received);

  // Local state for the unique placeholder preview premium modal 
  ...
  // Safe lengths check with fallback to 0
  const totalConnections = Array.isArray(connectionsList) ? connectionsList.length : 0;
  const totalPending = Array.isArray(pendingRequestsList) ? pendingRequestsList.length : 0;
  const totalProjects = Array.isArray(user.projects) ? user.projects.length : 0;
  ...
  return (
    <>
      ...
      <aside className="w-full h-[calc(100vh-6rem)] overflow-y-auto custom-scrollbar flex flex-col gap-5 pb-24 md:pb-6 pr-2">
        ...
        {/* QUICK STATS - DYNAMIC BACKEND VALUES */}
        <div className="flex-shrink-0 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-4 shadow-2xl grid grid-cols-2 gap-2.5">
          <div className="p-3 rounded-xl border border-white/5 bg-white/[0.02] text-center">
            ...
            <span className="text-lg font-bold text-gray-100 font-mono mt-0.5 inline-block">{totalConnections}</span>
          </div>
          <div className="p-3 rounded-xl border border-white/5 bg-white/[0.02] text-center">
            <span className="text-[9px] text-gray-400 uppercase tracking-wider font-medium block">Pending</span>
            <span className="text-lg font-bold text-amber-400 font-mono mt-0.5 inline-block">{totalPending}</span>
          </div>
          <div className="p-3 rounded-xl border border-white/5 bg-white/[0.02] text-center">
            <span className="text-[9px] text-gray-400 uppercase tracking-wider font-medium block">Projects</span>
            <span className="text-lg font-bold text-cyan-400 font-mono mt-0.5 inline-block">{totalProjects}</span>
          </div>
          <div className="p-3 rounded-xl border border-white/5 bg-white/[0.02] text-center">
            <span className="text-[9px] text-gray-400 uppercase tracking-wider font-medium block">Skills</span>
            <span className="text-lg font-bold text-purple-400 font-mono mt-0.5 inline-block">{totalSkills}</span>
          </div>
        </div>
        ...
        <div className="flex-shrink-0 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-2.5 shadow-2xl flex flex-col gap-1">
          ...
          {/* UPGRADE BUTTON WITH COMING SOON BADGE */}
          <button onClick={handlePremiumClick} className="flex items-center justify-between w-full px-4 py-2.5 text-xs font-semibold text-amber-400 bg-amber-500/5 border border-amber-500/10 rounded-xl hover:bg-amber-500/10 transition-all text-left">
            <div className="flex items-center gap-3">
              <FaCrown className="text-amber-500 text-sm" /> 
              <span>Upgrade to Premium</span>
            </div>
            <span className="text-[9px] bg-amber-500/20 text-amber-300 font-mono px-1.5 py-0.5 rounded border border-amber-500/30 uppercase tracking-tight shrink-0 font-bold">
              Soon
            </span>
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md transition-opacity">
          <div className="relative w-full max-w-sm rounded-2xl border border-amber-500/30 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="text-center mt-2">
              ...
              <h3 className="text-lg font-bold text-gray-100 tracking-tight">Premium Memberships</h3>
