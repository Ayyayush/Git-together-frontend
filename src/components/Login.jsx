    ...
    const delayDebounce = setTimeout(async () => {
      ...
      try {
        const res = await axios.get(
          `${BASE_URL}/user/check-username?username=${newUsername}`
        );
    ...
    try {
      ...
      setModalError(
        err.response?.data?.message ||
          "Failed to update your workspace profile routing parameters."
      );
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="relative w-full max-w-md rounded-2xl border border-indigo-500/30 bg-[#0F131F] p-8 shadow-2xl shadow-indigo-500/10 text-center animate-fade-in">
            ...
            <p className="text-sm text-gray-400 mt-2 mb-6">
              Your developer namespace migration is incomplete. Choose a
              persistent handle to continue.
            </p>
